...
public class BuildInfoResourceTest {
	...
	@Test
	public void shouldReturn500ErrorForBuildInfoWhenBuildInfoPropertiesFileIsNotAvailable() {
		try {
	...
	@Test
	public void shouldReturn500ErrorForVersionWhenBuildInfoPropertiesFileIsNotAvailable() {
		try {
			new BuildInfoResource().getVersion();
			fail("Expected a WebApplicationException to be thrown when build-info.properties not available.");
		} catch (WebApplicationException ex) {
			Response response = ex.getResponse();
	...
	public void shouldReturnVersionWhenBuildInfoPropertiesFileIsAvailable() {
		String version = new BuildInfoResource("/test-build-info.properties").getVersion();
